...
static void draw_header(lv_obj_t * calendar, const lv_area_t * mask)
{
    ...
    lv_draw_label(&header_area, mask, ext->style_header, opa_scale, txt_buf, LV_TXT_FLAG_CENTER, NULL, (uint16_t)-1, (uint16_t)-1, NULL);
    ...
    lv_draw_label(&header_area, mask, arrow_style, opa_scale, LV_SYMBOL_LEFT, LV_TXT_FLAG_NONE, NULL, (uint16_t)-1, (uint16_t)-1, NULL);
    ...
    lv_draw_label(&header_area, mask, arrow_style, opa_scale, LV_SYMBOL_RIGHT, LV_TXT_FLAG_NONE, NULL, (uint16_t)-1, (uint16_t)-1, NULL);
}
...
static void draw_day_names(lv_obj_t * calendar, const lv_area_t * mask)
{
    ...
    for(i = 0; i < 7; i++) {
        ...
        lv_draw_label(&label_area, mask, ext->style_day_names, opa_scale, get_day_name(calendar, i), LV_TXT_FLAG_CENTER,
                      NULL, (uint16_t)-1, (uint16_t)-1, NULL);
    }
}
...
static void draw_days(lv_obj_t * calendar, const lv_area_t * mask)
{
    ...
    for(week = 0; week < 6; week++) {
        ...
        for(day = 0; day < 7; day++) {
            ...
            lv_draw_label(&label_area, mask, final_style, opa_scale, buf, LV_TXT_FLAG_CENTER, NULL, (uint16_t)-1, (uint16_t)-1, NULL);

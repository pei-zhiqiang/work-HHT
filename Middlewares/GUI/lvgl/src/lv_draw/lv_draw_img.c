                 ...
                 lv_opa_t opa_scale)
{
    if(src == NULL) {
        ...
        lv_draw_label(coords, mask, &lv_style_plain, LV_OPA_COVER, "No\ndata", LV_TXT_FLAG_NONE, NULL, (uint16_t)-1, (uint16_t)-1, NULL);
        return;
    }
                                 ...
                                 const lv_style_t * style, lv_opa_t opa_scale)
{
    ...
    if(cdsc->dec_dsc.error_msg != NULL) {
        LV_LOG_WARN("Image draw error");
        lv_draw_rect(coords, mask, &lv_style_plain, LV_OPA_COVER);
        lv_draw_label(coords, mask, &lv_style_plain, LV_OPA_COVER, cdsc->dec_dsc.error_msg, LV_TXT_FLAG_NONE, NULL, (uint16_t)-1,
                      (uint16_t)-1, NULL);
    }
